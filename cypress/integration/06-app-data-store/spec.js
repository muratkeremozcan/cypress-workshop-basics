...
it('has vuex store', () => {
  ...
  cy.window().its('app.$store')
    ...
    .its('state').should('be.an', 'object')
    .its('todos').should('be.an', 'array')
})
...
it('creates items with deterministic ids 1 and 2 using a stub, and checks store', () => {
  ...
  cy.window().its('Math').then(Math => 
    cy.stub(Math, 'random').callsFake(()=> `0.${count++}`)).as('random')
  ...
  cy.window().its('app.$store.state.todos')
...
const storeDispatch = (args) => cy.window().its('app.$store').invoke('dispatch', ...args)
